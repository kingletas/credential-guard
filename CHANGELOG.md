...
- **A credential written without quotes is caught wherever it sits.** `sudo NEW_RELIC_API_KEY=… newrelic install` pasted into a note, a Dockerfile `RUN`, or a README passed, because the unquoted rule only looked at a `.env` or shell file whose whole line was the assignment. Upper-case names now count anywhere on a line, when the credential word is a whole part of the name, so `max_tokens=…` and `TOKENIZER=…` still pass. So does a name that points at a credential rather than holding one: `…_ID`, `…_REF`, `…_ARN`, `…_URL`, `…_FILE`, `…_PATH`, `…_NAME`. `--password=…` style flags and unquoted `password: …` values in YAML are caught the same way. A value read from the environment (`$VAR`, `${{ secrets.X }}`, `{{ vault_x }}`, `!vault`) still passes.
- **Twenty more provider key formats:** New Relic API and license keys, OpenAI project keys, GitLab, Atlassian and Bitbucket tokens, Google OAuth client secrets, Azure storage account keys, Slack and Discord webhook URLs, Telegram bot tokens, Shopify, Mailgun, npm (including `_authToken=` in `.npmrc`), PyPI, Docker Hub and Hugging Face tokens, and PGP and PuTTY private keys. `license_key` and `install_key` now count as credential-shaped names.
- **`.git-credentials`, `.netrc`, `.pgpass` and `.htpasswd` are refused by name**, like `.env`.
...
- **`watch` handles a watchlist of any size.** A watchlist naming a large directory blew past ARG_MAX at 35,541 files, and the wrapper reported "Argument list too long" **as a credential finding**. The list now reaches the scanner in one call on standard input, and a scanner that couldn't run exits 2 and says nothing was checked, which is a different thing from a scanner that looked and found something.
...
- **`watch` no longer reports a credential when it only failed to open a file.** A watched file too large to scan failed the run, which is right, but the summary said *a credential is sitting in a file that should not hold one* with no credential listed, and `--notify` raised the same alert. The summary and the notification now say which happened.
...
- **A Magento-encrypted value such as `0:3:…` no longer fails the scan.** That's ciphertext from Magento's encryptor, unreadable without the store's crypt key, whether it's stored as-is, escaped in JSON, or written to a shell history. `watch` was reporting one in a PHP shell history.
- **The README no longer lists JWTs among the formats it refuses.** The scanner has no JWT pattern and never had one.
- **`scan` works on a directory of any size.** A tree with more files than fit on one command line, such as a whole store, failed with *Argument list too long*, and that exit looked exactly like a finding. The file list now reaches the scanner on standard input, through a new `--files0-from LIST` option that reads NUL-separated names (`-` for standard input).
- **`history` works on a repository of any size.** It passed every blob on one command line too, so a large history failed the same way.
- **A scanner that couldn't run is no longer reported as a finding**, in `scan`, `history` or `watch`. A missing scanner, one killed mid-run, or one that crashed used to exit 1, the same as a credential; each now exits 2 and says nothing was checked.
- **A directory that can't be read fails `scan` and `watch`.** Its files were skipped and the run could still exit 0, which broke the promise that exit 0 means every file was opened. It now exits 1 and names what couldn't be read.
- **A directory scan of a large tree is faster.** The file list was read one byte at a time; 100,000 files took about 35 seconds and now take under 15 on the same machine.
...
- **A value is reported once per line**, however many rules describe it. A PostgreSQL URL with a password, or a GitHub token under a `"password"` key, used to produce two findings for one secret.
