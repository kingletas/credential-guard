...
- **A scanner that couldn't run is no longer reported as a finding.** A missing scanner, one killed mid-run, or one that crashed used to exit 1, the same as a credential; each now exits 2 and says nothing was checked.
