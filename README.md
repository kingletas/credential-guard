...
**Exit 0 means every file was opened and none looked like a credential — nothing else.** A scan that found something, or that couldn't read a file or a directory, exits 1; one given a target that doesn't exist, a file list it could open none of, or a scanner that couldn't run at all, exits 2. A security check whose silence can also mean *I didn't look* is worse than no check, because it still prints a green tick.
