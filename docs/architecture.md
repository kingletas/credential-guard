...
This is vendored into other people's repositories and runs as a pre-commit hook. **A dependency here is a dependency in every repository it's copied into**, and a hook that fails to import is a hook somebody removes rather than fixes.
...
An error message has to be safe to paste into a bug report, a CI log, or a screenshot. **A hook whose output you can't share is a hook people bypass instead of reading.**
...
- **By filename**, because the canonical copy and a vendored copy are different files at different paths. Matching on the resolved path of the running file wouldn't recognise the copy being scanned.
...
One directory argument walks the whole tree. That's right for a repository sweep.

Several arguments — or one that's a file — scan exactly those. That's right for a pre-commit hook, which is handed the staged files. **Re-scanning twelve thousand files on every commit is how a hook gets uninstalled.**

Inside a git repository the directory walk uses `git ls-files` rather than `find`: an untracked build directory isn't going anywhere, and scanning `node_modules` teaches people to ignore the output.
...
- The tree holds **every blob in the repository**, credentials included. It's removed by a `trap` rather than by a line at the end, so a scan that exits early — a failed `cat-file`, a Ctrl-C — doesn't leave that on disk.
...
It walks `cat-file --batch-all-objects`, not `rev-list HEAD`: an object on a deleted branch, or in a commit that was amended away, is still an object in the repository until it's garbage collected.

## `install` vendors rather than references

A pre-commit hook has to run for **everyone who clones the repository**, and a contributor doesn't have your `PATH`. So `install` copies the scanner to `scripts/check_credentials.py` in the target repo.
...
- **pre-commit present, not wired** — print the hook block to add. It doesn't edit somebody's config file for them.
...
**A regex over a diff can't make a repository free of credentials.** A key split across two lines, base64'd, or in a format nobody has written a pattern for will get through.

The tool's error message says the important half: **rotate first, clean the history second.** Rewriting history doesn't recall a copy someone has already cloned, doesn't un-index a page a scraper already read, and doesn't expire a token. The rewrite is housekeeping; the rotation is the fix.
